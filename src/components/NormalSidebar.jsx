...
const NormalSidebar = ({ onSearchClick, onNotificationClick }) => {
  ...
  const [activeMenu, setActiveMenu] = useState('home'); // default 'home' sebagai active

  const handleSearchClick = () => {
    console.log('NormalSidebar: Search button clicked');
    setActiveMenu('search'); // set search sebagai active
    if (onSearchClick) {
      onSearchClick();
    }
  };
  ...
  const handleNotificationClick = () => {
    ...
    setActiveMenu('notification'); // set notification sebagai active
  ...
  const handleHomeClick = () => {
    console.log('NormalSidebar: Home button clicked');
    setActiveMenu('home'); // set home sebagai active
    // Jika ada callback untuk home, panggil di sini
  };

  const handleExploreClick = () => {
    console.log('NormalSidebar: Explore button clicked');
    setActiveMenu('explore'); // set explore sebagai active
    // Jika ada callback untuk explore, panggil di sini
  };

  const handleCreateClick = () => {
    console.log('NormalSidebar: Create button clicked');
    setActiveMenu('create'); // set create sebagai active
    // Jika ada callback untuk create, panggil di sini
  };

  const handleProfileClick = () => {
    console.log('NormalSidebar: Profile button clicked');
    setActiveMenu('profile'); // set profile sebagai active
    // Jika ada callback untuk profile, panggil di sini
  };
  ...
  return (
    <div className="w-[244px] 2xl:w-[335px] h-[1024px] 2xl:h-[1080px] px-3 py-6 bg-black flex flex-col justify-start items-start gap-6 overflow-hidden z-20">
      ...
      <div className="w-full flex flex-col justify-start items-start gap-1">
        {/* Home - dengan kondisi active dan SVG yang berubah */}
        <div className="w-[244px] 2xl:w-[335px] h-12 p-3 bg-black rounded-lg flex justify-start items-center gap-2.5 overflow-hidden cursor-pointer hover:bg-neutral-800 transition-colors" onClick={handleHomeClick}>
          <div className="py-2 flex justify-start items-center gap-2.5">
            {activeMenu === 'home' ? (
              // SVG untuk state active (filled home)
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                <path fill="#fff" stroke="#fff" d="M12 .5c.506 0 .952.118 1.348.352l.167.107.003.002 9 6.172.003.003c.271.181.489.403.655.668l.067.116c.152.276.235.563.253.863l.004.13v12.344c0 .53-.175.99-.548 1.397l-.172.172a2.49 2.49 0 0 1-1.521.663L21 23.5h-4.5c-.273 0-.485-.071-.658-.203l-.072-.06a.772.772 0 0 1-.266-.52l-.004-.09v-6.855c0-.464-.156-.874-.457-1.209l-.139-.137c-.34-.31-.748-.48-1.205-.518l-.198-.008H10.5c-.47 0-.897.13-1.256.402l-.15.125a1.77 1.77 0 0 0-.584 1.148l-.01.196v6.858c0 .216-.065.39-.205.544l-.065.064a.987.987 0 0 1-.616.26l-.113.003H3c-.617 0-1.141-.173-1.592-.517l-.188-.157c-.49-.448-.72-.963-.72-1.57V8.915c0-.305.065-.594.197-.874l.06-.12c.153-.278.354-.512.61-.705l.112-.08.003-.002 9-6.172.003-.002A2.574 2.574 0 0 1 12 .5Z"/>
              </svg>
            ) : (
              // SVG untuk state normal (outline home)
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                <path stroke="#fff" strokeWidth="2.5" d="M12 1.25c.38 0 .694.087.97.25l.116.074.007.005 9 6.172.01.007c.16.106.287.23.39.373l.093.15c.085.156.135.31.154.469l.01.161v12.346c0 .334-.1.619-.355.896l-.12.12c-.306.278-.65.436-1.083.47l-.191.007H16.5c-.107 0-.16-.02-.193-.042l-.03-.024a.213.213 0 0 1-.025-.027v.001l-.002-.033v-6.854c0-.648-.224-1.235-.646-1.706l-.194-.194a2.747 2.747 0 0 0-1.645-.71l-.262-.01H10.5c-.619 0-1.206.175-1.705.55l-.208.173a2.518 2.518 0 0 0-.822 1.621l-.015.274v6.86a.192.192 0 0 1-.002.03l-.023.025a.221.221 0 0 1-.133.06l-.09.006H3c-.46 0-.825-.123-1.142-.366l-.132-.111c-.301-.276-.44-.554-.47-.877l-.006-.14V8.914c0-.165.03-.322.092-.477l.074-.156c.086-.157.195-.294.333-.412l.148-.111.01-.007 9-6.172.007-.005A1.825 1.825 0 0 1 12 1.25Z"/>
              </svg>
            )}
            <div className={`justify-start text-base font-['Roboto'] ${activeMenu === 'home' ? 'text-white font-semibold' : 'text-white font-semibold'}`}>
              Home
            </div>
          </div>
        </div>

        {/* Search - dengan kondisi active */}
        <div className="w-[244px] 2xl:w-[335px] h-12 p-3 bg-black rounded-lg flex justify-start items-center gap-2.5 overflow-hidden cursor-pointer hover:bg-neutral-800 transition-colors" onClick={handleSearchClick}>
          <div className="py-2 flex justify-start items-center gap-2.5">
            ...
            <div className={`justify-start text-base font-['Roboto'] ${activeMenu === 'search' ? 'text-white font-semibold' : 'text-white font-normal'}`}>
              Search
            </div>
          </div>
        </div>

        {/* Explore - dengan kondisi active */}
        <div className="w-[244px] 2xl:w-[335px] h-12 p-3 bg-black rounded-lg flex justify-start items-center gap-2.5 overflow-hidden cursor-pointer hover:bg-neutral-800 transition-colors" onClick={handleExploreClick}>
          <div className="py-2 flex justify-start items-center gap-2.5">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <div className={`justify-start text-base font-['Roboto'] ${activeMenu === 'explore' ? 'text-white font-semibold' : 'text-white font-normal'}`}>
              Explore
            </div>
          </div>
        </div>
        ...
        <div className="w-[244px] 2xl:w-[335px] h-12 p-3 bg-black rounded-lg flex justify-start items-center gap-2.5 overflow-hidden cursor-pointer hover:bg-neutral-800 transition-colors" onClick={handleNotificationClick}>
          <div className="py-2 flex justify-start items-center gap-2.5">
            ...
            <div className={`justify-start text-base font-['Roboto'] ${activeMenu === 'notification' ? 'text-white font-semibold' : 'text-white font-normal'}`}>
              Notifications
            </div>
          </div>
        </div>

        {/* Create - dengan kondisi active */}
        <div className="w-[244px] 2xl:w-[335px] h-12 p-3 bg-black rounded-lg flex justify-start items-center gap-2.5 overflow-hidden cursor-pointer hover:bg-neutral-800 transition-colors" onClick={handleCreateClick}>
          <div className="py-2 flex justify-start items-center gap-2.5">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <div className={`justify-start text-base font-['Roboto'] ${activeMenu === 'create' ? 'text-white font-semibold' : 'text-white font-normal'}`}>
              Create
            </div>
          </div>
        </div>
          
        {/* Profile - dengan kondisi active */}
        <div className="w-[244px] 2xl:w-[335px] h-12 p-3 bg-black rounded-lg flex justify-start items-center gap-2.5 overflow-hidden cursor-pointer hover:bg-neutral-800 transition-colors" onClick={handleProfileClick}>
          <div className="py-2 flex justify-start items-center gap-2.5">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <circle cx="12" cy="12" r="12" fill="white"/>
            </svg>
            <div className={`justify-start text-base font-['Roboto'] ${activeMenu === 'profile' ? 'text-white font-semibold' : 'text-white font-normal'}`}>
              Profile
            </div>
          </div>
        </div>
